...
export function App() {
  ...
  // A scanned stamp deep-links as ?h=<sha256>. Remember it once so VerifyPanel
  // can pre-load the ledger check and demand the actual file before trusting.
  const [scannedHash, setScannedHash] = useState<string | null>(() => loadHashParam());
  ...
  return (
    <div className="app">
      ...
      <main className="shell app__main">
        {view === "verify" && <PublicView scannedHash={scannedHash} onScannedConsumed={() => setScannedHash(null)} />}
...
/** Read a strict 64-hex `?h=` param, used by QR scan stamps. Anything else
 *  (or anything scannable-looking but malformed) is ignored — the QR URL is
 *  generated by us and validated here, so a forged code can only ever open
 *  the normal page. */
function loadHashParam(): string | null {
  try {
