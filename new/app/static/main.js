...
let latencyChartObj = null;
    ...
    function renderVerificationRow(res, name, rawBlob) {
        ...
        // AI-detection explainer: which model ran, at what confidence, how fast,
        // and what that means in plain words — so even a non-technical judge can
        // trust the verdict.
        //   - provider "document": a scanned/text page — show a distinct notice
        //     pointing trust to the signature/provenance (cloud AI models misfire
        //     on photos-of-paper, so we deliberately don't run them).
        //   - otherwise when a model actually inspected the image.
        let aiPanel = "";
        const aiDet = data.ai_detection || {};
        const providerKey = data.ai_provider || aiDet.provider || "";
        if (providerKey === "document") {
            const docExpl = data.ai_explanation || aiDet.explanation ||
            ...
            aiPanel = `
                <div class="forensic-panel ai-panel doc-panel">
                    <div class="forensic-head">SCANNED DOCUMENT <span class="bullet">•</span> document-aware pre-check <span class="chip cyan">document</span></div>
                    <div class="ai-explain">${esc(docExpl)}</div>
                </div>`;
        } else if (aiDet.ran || aiDet.ai_score) {
            const modelName = data.ai_model || aiDet.model || "built-in detector";
            const providerTag = providerKey
                ? `<span class="chip cyan">${esc(providerKey)}</span>`
                : "";
            const aiScore = Number.isFinite(data.ai_score) ? data.ai_score : (aiDet.ai_score || 0);
            const latency = aiDet.latency_ms || 0;
            const pct = `<span class="forensic-conf">AI confidence ${aiScore}%</span>`;
            const latTag = latency ? `<span class="chip">${latency} ms</span>` : "";
            const expl = data.ai_explanation || aiDet.explanation ||
                (aiScore >= 50 ? "The detector thinks this image was machine-generated." :
                                 "The detector saw no strong AI-generation signature.");
            aiPanel = `
                <div class="forensic-panel ai-panel">
                    <div class="forensic-head">AI CONTENT DETECTION <span class="bullet">•</span> ${esc(modelName)}${providerTag}${pct}${latTag}</div>
                    <div class="ai-explain">${esc(expl)}</div>
                </div>`;
        }
    ...
    async function loadAnalytics() {
        ...
        renderLatencyChart(latency, providers);
        if (currentAnalyticsScope === "global") await loadAiQuota();
    }
    ...
    async function loadAiQuota() {
        ...
        try {
            ...
            if (!res.ok) return;
            const u = res.data;
            const pct = Math.min(Math.round((u.ops_used_month / u.limit_month) * 100), 100);
            const low = u.remaining_month <= 200;
            box.style.display = "block";
            box.innerHTML =
                `AI DETECTION BUDGET &mdash; ${esc(String(u.provider || "").toUpperCase())} (${esc(u.model || "")})<br>` +
                `<b style="color:${low ? "var(--danger,#fb3a6b)" : "var(--success,#22e3a4)"}; font-size:15px;">${u.remaining_month} uses left</b>` +
                ` this month (${pct}% of ${u.limit_month} budget used) &middot; ${u.remaining_today} left today` +
                `<br><span style="color:var(--text-lo); font-size:10px;">Counted server-side; each image check consumes Sightengine operations. No key or vendor data is exposed.</span>`;
    ...
    function renderLatencyChart(latency, providers) {
        ...
        // No samples yet (local scope or empty DB) -> show an empty-state label.
        if (!latency) {
            ctx.save();
            ctx.font = "12px JetBrains Mono";
            ctx.fillStyle = "#a5adc0";
            ctx.textAlign = "center";
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            ctx.fillText("No AI detections captured yet. Verify an image to record latency.",
                canvas.width / 2, canvas.height / 2);
            ctx.restore();
            return;
        }
        latencyChartObj = new Chart(ctx, {
            type: "bar",
            data: {
                labels: ["Average", "Fastest", "Slowest"],
                datasets: [{
                    label: "ms",
                    data: [latency.avg_ms, latency.min_ms, latency.max_ms],
                    backgroundColor: ["rgba(167,139,250,0.8)", "rgba(34,227,164,0.8)", "rgba(226,90,52,0.8)"],
                    borderRadius: 4
                }],
            },
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: { label: (i) => `${i.parsed.y} ms over ${latency.samples} image(s)` }
                    }
                },
                scales: {
                    y: { beginAtZero: true, grid: { color: "rgba(255,255,255,0.05)" }, ticks: { color: "#a5adc0" } },
